...
// Central configuration for geometry, IO pins, joint/servo conventions and limits.
// Keep this header lightweight and purely constexpr.
namespace cfg {
...
// Workspace limits
static constexpr float R_MIN = 0.0f;
...
// where jointDeg is q in degrees.
...
static constexpr float jointRadFromServoDeg(const float servoDeg, const float zeroDeg, int sign) {
  // jointDeg = (servoDeg - zeroDeg)/sign
  return ((servoDeg - zeroDeg) / static_cast<float>(sign)) * DEG2RAD;
}

static constexpr float Q1_A = jointRadFromServoDeg(SERVO_MIN_DEG, BASE_ZERO_DEG, BASE_SIGN);
static constexpr float Q1_B = jointRadFromServoDeg(SERVO_MAX_DEG, BASE_ZERO_DEG, BASE_SIGN);
static constexpr float Q1_MIN = (Q1_A < Q1_B) ? Q1_A : Q1_B;
static constexpr float Q1_MAX = (Q1_A < Q1_B) ? Q1_B : Q1_A;

static constexpr float Q2_A = jointRadFromServoDeg(SERVO_MIN_DEG, SHOULDER_ZERO_DEG, SHOULDER_SIGN);
static constexpr float Q2_B = jointRadFromServoDeg(SERVO_MAX_DEG, SHOULDER_ZERO_DEG, SHOULDER_SIGN);
static constexpr float Q2_MIN = (Q2_A < Q2_B) ? Q2_A : Q2_B;
static constexpr float Q2_MAX = (Q2_A < Q2_B) ? Q2_B : Q2_A;

static constexpr float Q3_A = jointRadFromServoDeg(SERVO_MIN_DEG, ELBOW_ZERO_DEG, ELBOW_SIGN);
static constexpr float Q3_B = jointRadFromServoDeg(SERVO_MAX_DEG, ELBOW_ZERO_DEG, ELBOW_SIGN);
static constexpr float Q3_MIN = (Q3_A < Q3_B) ? Q3_A : Q3_B;
static constexpr float Q3_MAX = (Q3_A < Q3_B) ? Q3_B : Q3_A;

// --- Motion limits -----------------------------------------------------
static constexpr float MAX_SPEED_DEG_PER_S = 60.0f;
static constexpr float MAX_ACC_DEG_PER_S2  = 120.0f;
static constexpr float LOOP_DT_S           = 0.02f;  // nominal loop time
...
static constexpr float TRAJ_SEG_TIME_S = 0.5f;

// Optional startup calibration loop.
